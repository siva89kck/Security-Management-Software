...
$(document).on("click", ".menu-previous", function (e) {
    ...
    let attribute =
        layoutOption == "ltr" || layoutOption == "box-layout"
            ? "marginLeft"
            : "marginRight";
...
$(document).on("keyup", ".search-filter", function (e) {
    ...
    $(".search-list").each(function () {
        $(this)
            .find(".search-list-content h6")
            .each(function (ele) {
                var content = $(this).text().toLowerCase();
                if (content.includes(search)) {
                    $(this).closest(".search-list-item").show();
                    var highlightedText = content.replace(
                        new RegExp(search, "gi"),
                        function (match) {
                            return (
                                '<span class="highlight-searchtext">' +
                                match +
                                "</span>"
                            );
                        }
