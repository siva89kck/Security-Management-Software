                    ...
                    <div class="row g-3">
                        <div class="col-md-3">
                            <div class="detail-card">
                                ...
                                <input type="date" name="issue_date" value="{{ old('issue_date', $issue->issue_date) }}"
                    ...
                    <div class="table-responsive">
                        <table class="table items-table align-middle" id="items-table">
                            ...
                            <tbody>
                                ...
                                <tr>
                                    <td>
                                        <select name="items[{{ $index }}][uniform_master_id]" class="form-select item-select" required>
                                            <option value="">-- Select --</option>
                                            @foreach ($masters as $master)
                                            <option value="{{ $master->id }}" data-price="{{ $master->price }}"
                                                data-size="{{ $master->size ?? '' }}"
        ...
        tr.innerHTML = `
            <td>
                <select name="items[${idx}][uniform_master_id]" class="form-select item-select" required>
                    <option value="">-- Select --</option>
                    ${masters.map(m=>`<option value="${m.id}" data-price="${m.price}" data-size="${m.size||''}" ${m.id==defaultItem.id?'selected':''}>${m.name}</option>`).join('')}
                </select>
            </td>
            <td>
            ...
            <td>
                <input type="number" step="0.01" name="items[${idx}][price]" class="form-control price" value="${price}">
            </td>
            <td class="total-cell">${(1 * price).toFixed(2)}</td>
            <td><button type="button" class="btn btn-sm btn-danger remove">x</button></td>
        `;
        return tr;
    }
