...
<div class="enclosure-form-repeater mt-3" id="docRepeater">
    @php
        $enclosures = old('enclosures', isset($employee) ? $employee->enclosures->map(function($d){
            return [
        ...
        })->toArray() : [['document_type'=>'','original_copy'=>'','proof_no'=>'','file_path'=>'']]);
    @endphp
        ...
        <div class="col-md-12 d-flex justify-content-end">
            @if($i==0)
                <button type="button" class="btn btn-outline-primary btn-sm enclosure-add-row">
                    <i class="bi bi-plus-lg me-1"></i> Add Enclosure
                </button>
            @else
...
document.addEventListener("DOMContentLoaded", function () {
    ...
    let index = repeater.querySelectorAll('.repeater-item').length;

    repeater.addEventListener("click", function (e) {
        if (e.target.classList.contains("enclosure-add-row")) {
        ...
        if (e.target.classList.contains("enclosure-remove-row")) {
