        ...
        <div class="col-12">
                                        ...
                                        <div class="employee-profile-header">
                                            <div class="profile-image-container">
                                                @if (!empty($employee->photo))
                                                    <img src="{{ asset('storage/' . $employee->photo) }}"
                                                        class="profile-image" alt="Employee Photo">
                                                @else
                                                    <img src="http://security-software.test/assets/images/avtar/woman.jpg"
                                                        class="profile-image" alt="User Avatar">
                                                @endif
                                                <span
                                                    class="status-badge {{ $employee->status == 'active' ? 'status-active' : 'status-inactive' }}"></span>
                                                <div>
                                            ...
                                            <div class="employee-basic-info">
                                                ...
                                                <div class="employee-id">ID:
                                                    EMP-{{ str_pad($employee->id, 4, '0', STR_PAD_LEFT) }}</div>
                                                <div class="employee-position">
                                                    {{ optional($employee->officialDetail)->role ?? 'Not specified' }}</div>
                                                <div class="employee-department">
                                                    {{ optional($employee->officialDetail)->employee_type ?? 'Not specified' }}
                                                </div>
                                            </div>
                                        </div>
            ...
            'ESI Calculation' => isset($employee->officialDetail->esi_calculation) ? ($employee->officialDetail->esi_calculation ? 'Yes' : 'No') : '-',
                                            ...
                                            <div class="detail-card mb-3">
                                                <div class="row">
                                                    ...
                                                    <div class="col-md-3 mb-2">
                                                        ...
                                                        <div class="value">{{ !empty($lang['read']) ? 'Yes' : 'No' }}
                                                        </div>
                                                    </div>
                                                    <div class="col-md-3 mb-2">
                                                        <div class="label">Write</div>
                                                        <div class="value">{{ !empty($lang['write']) ? 'Yes' : 'No' }}
                                                        </div>
                                                    </div>
                                                    <div class="col-md-3 mb-2">
                                                        <div class="label">Speak</div>
                                                        <div class="value">{{ !empty($lang['speak']) ? 'Yes' : 'No' }}
                                                        </div>
                                                    </div>
                                                </div>
                                            </div>
                                    ...
                                    <div class="tab-pane fade" id="tab-family">
                                        ...
                                        @empty
                                            <div class="detail-card">
                                                <div class="value text-center text-muted py-3">No family members available
                                                </div>
                                            </div>
                                        @endforelse
                                    </div>
                                    ...
                                    <div class="tab-pane fade" id="tab-experiences">
                                        ...
                                        @empty
                                            <div class="detail-card">
                                                <div class="value text-center text-muted py-3">No experiences available
                                                </div>
                                            </div>
                                        @endforelse
                                    </div>
        ...
        'ESI Calculation' => isset($employee->officialDetail->esi_calculation) ? ($employee->officialDetail->esi_calculation ? 'Yes' : 'No') : '-',
            ...
            'Other Allowance %' => optional($payslip)->other_allowance,
                                    ...
                                    <div class="tab-pane fade" id="tab-banks">
                                        ...
                                        @empty
                                            <div class="detail-card">
                                                <div class="value text-center text-muted py-3">No bank details available
                                                </div>
                                            </div>
                                        @endforelse
                                    </div>
...
function printEmployeeInfo() {
    ...
    // 1️⃣ Company Logo + Title
    var logoUrl = "{{ asset('assets/images/logo/RSS_Security_Logo.png') }}";
    ...
    // 2️⃣ Profile Section
    var profile = document.querySelector('.employee-profile-header');
    if (profile) {
        // profile image
        var profileImg = profile.querySelector('img')?.src || ''; // உங்கள் profile image selector
        var name = profile.querySelector('h3')?.innerText || '-';
        var empId = profile.querySelector('.employee-id')?.innerText || '-';
        var role = profile.querySelector('.employee-position')?.innerText || '-';
        var type = profile.querySelector('.employee-department')?.innerText || '-';

        combinedContent += `
        <div class="profile-block" style="display:flex;align-items:center;margin-bottom:20px;">
            <div style="flex:0 0 120px;text-align:center;">
                ${ profileImg
                    ? `<img src="${profileImg}" style="width:100px;height:100px;object-fit:cover;border-radius:8px;border:1px solid #ccc;">`
                    : '<div style="width:100px;height:100px;background:#eee;border-radius:8px;display:flex;align-items:center;justify-content:center;font-size:12px;color:#888;">No Photo</div>'}
            </div>
    ...
    // 3️⃣ All Tab Panes (skip empty sections)
    var allTabs = document.querySelectorAll('.tab-pane');
    allTabs.forEach(function(tab) {
        if (!tab.innerHTML.trim()) return;

        var sectionTitle = tab.querySelector('.section-title')
            ? tab.querySelector('.section-title').innerText : '';
        var cards = tab.querySelectorAll('.detail-card');

        // skip if no cards or no content
        if (cards.length === 0) return;

        // Section title
        if (sectionTitle) combinedContent += `<h3>${sectionTitle}</h3>`;

        cards.forEach(function(card) {
            var allLabels = card.querySelectorAll('.label');
            var allValues = card.querySelectorAll('.value');

            for (let i = 0; i < allLabels.length; i++) {
                var label = allLabels[i]?.innerText || '';
                // ✅ use innerHTML to preserve <a> / View Document link / icons
                var value = allValues[i]?.innerHTML || '';
                if (value.trim() !== '') { // show only if value is not empty
                    combinedContent += `<p><strong>${label}</strong> ${value}</p>`;
                }
            }
        });
    ...
    a.document.write(`
        ...
        h2 { text-align:center; color:#333; margin-bottom:0; }
        h3 { color:#4a6cf7; border-bottom:1px solid #ddd; padding-bottom:5px; margin-top:20px; margin-bottom:10px; font-size:16px; }
        p { margin:3px 0; font-size:12px; line-height:1.4; }
        p strong { width:180px; display:inline-block; font-weight:600; color:#000; vertical-align:top; }
        hr { border:none; border-top:1px dashed #ccc; margin:8px 0; }
        img { max-height:80px; }
        .profile-block { page-break-inside:avoid; }
        /* print link style */
        a, a:visited { color:#007bff; text-decoration:none; font-size:11px; }
